...
import { format, parse, startOfWeek, getDay,nextDay, startOfToday, type Day } from 'date-fns';
import { enUS } from 'date-fns/locale/en-US';
import {Calendar, Users, MessageSquare, Phone, Video, CheckCircle, XCircle, Activity, TrendingUp,} from 'lucide-react';
import {
...
export const DoctorDashboard: React.FC = () => {
  ...
  const weekdays: Record<Weekday, number> = {sunday: 0,monday: 1,tuesday: 2,wednesday: 3,thursday: 4,friday: 5,saturday: 6,};
  ...
  return appointments
    .map((apt: any) => {
      ...
      // Compute next date for the given weekday
      const today = startOfToday();
      const appointmentDate = nextDay(today, dayIndex as Day); // date-fns

      // Construct start DateTime from date and time
      const dateStr = appointmentDate.toISOString().split("T")[0]; // 'YYYY-MM-DD'
      const start = new Date(`${dateStr}T${startTime}`);
      const duration = (apt.duration ?? 30) * 60000;

      return {
        title: `${apt.user?.firstName ?? "Unknown"} – ${apt.type ?? "Consultation"}`,
        start,
  ...
  const appointmentStatusData = useMemo(() => {
    const counts: Record<string, number> = {};
    appointments.forEach((apt: any) => {
      const st = apt.appointmentStatus || 'Unknown';
      counts[st] = (counts[st] || 0) + 1;
    });
    const colorMap: Record<string,string> = {
      Completed:'#10B981', Scheduled:'#3B82F6', Cancelled:'#EF4444',
      Confirmed:'#8B5CF6', Pending:'#F59E0B', Unknown:'#9CA3AF'
    };
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <StatCard
            ...
            subtitle={`${appointments.filter((a: any) => a.appointmentStatus === 'Confirmed').length} confirmed, ${appointments.filter((a: any) => a.appointmentStatus === 'Pending').length} pending`}
            icon={Calendar} bgColor="bg-blue-50" iconColor="text-blue-600"
          />
          <StatCard title="Total Patients" value={recentPatients.length.toString()} icon={Users} bgColor="bg-green-50" iconColor="text-green-600" />
          <StatCard title="This Month" value={patientVisitsData.find(m => m.month === format(new Date(), 'MMM'))?.visits.toString() || '0'} subtitle="Patient visits" changeType="increase" icon={Activity} bgColor="bg-purple-50" iconColor="text-purple-600" />
          <StatCard title="Unread Messages" value={appointments.filter((a: any) => a.appointmentStatus === 'Pending').length.toString()} icon={MessageSquare} bgColor="bg-orange-50" iconColor="text-orange-600" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="flex justify-end space-x-2">
              <button onClick={() => setViewMode('list')} className={`px-4 py-2 rounded-full text-sm transition ${viewMode === 'list' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>List View</button>
              <button onClick={() => setViewMode('calendar')} className={`px-4 py-2 rounded-full text-sm transition ${viewMode === 'calendar' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>Calendar View</button>
            </div>
            ...
            {viewMode === 'list' ? (
              <div className="bg-white rounded-xl p-6 shadow-sm border space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <div className="space-x-2">
                    <button onClick={() => setActiveView('today')} className={`px-3 py-1 rounded-full text-sm transition ${activeView === 'today' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>Today</button>
                    <button onClick={() => setActiveView('week')} className={`px-3 py-1 rounded-full text-sm transition ${activeView === 'week' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>This Week</button>
                  </div>
                </div>
                ...
                {activeView === 'today' ? (
                  appointments.length ? (
                    appointments.map((apt: any) => (
                      <div key={apt.appointmentId} className="border p-4 rounded-lg flex justify-between items-center hover:bg-gray-50 transition">
                        <div className="flex items-center space-x-4">
                          ...
                          <div>
                            <h3 className="font-semibold">{apt.user?.firstName} {apt.user?.lastName}</h3>
                        ...
                        <div className="text-right text-sm">
                          ...
                          {apt.appointmentStatus === 'Confirmed' ? <CheckCircle className="text-green-500 w-4 h-4 mt-1" /> : <XCircle className="text-yellow-500 w-4 h-4 mt-1" />}
                        </div>
              ...
              <div className="bg-white rounded-xl p-6 shadow-sm border h-[500px]">
                <h3 className="font-semibold mb-2">Appointments Calendar</h3>
                <BigCalendar localizer={localizer} events={calendarEvents} startAccessor="start" endAccessor="end" style={{ height: '100%' }} />
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white p-6 rounded-xl border shadow-sm">
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <PieChart>
                    <Pie data={appointmentStatusData} cx="50%" cy="50%" dataKey="value" innerRadius={40} outerRadius={80} paddingAngle={5}>
                      {appointmentStatusData.map((e, i) => <Cell key={i} fill={e.color} />)}
                    </Pie>
                    <Tooltip />
                  </PieChart>
                </ResponsiveContainer>
