...
export const AdminDashboard = () => {
  ...
  const appointmentStatusCounts =
    appointmentData?.reduce((acc: any, appointment: any) => {
      const status = appointment.appointmentStatus || 'Unknown';
      acc[status] = (acc[status] || 0) + 1;
      return acc;
    }, {} as Record<string, number>) || {};

  const appointmentStatusData = Object.keys(appointmentStatusCounts).map(
    (status) => ({
      name: status,
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <StatCard
            title="Appointments"
            value={
              appointmentsLoading ? 'Loading...' : totalAppointments.toString()
            }
            change="+12.5"
            changeType="increase"
            icon={ClipboardList}
